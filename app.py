        ...
        try:
            ...
            if _aecfg is not None and getattr(_aecfg, "enabled", False):
                ...
                def _fetch_last_with_fallback(sym: str):
                    ...
                    if q is not None:
                        return q
                    # Fallback: last known price from most recent scan results
                    return LAST_PRICE_CACHE.get(str(sym).upper().strip())
                ...
                reconcile_and_execute(
                    ...
                    allow_powerhour,
                    allow_afterhours,
                    _fetch_last_with_fallback,
                )
                # Place entries for already-staged lifecycles (pre-scan pass)
                try_send_entries(_aecfg, allow_opening, allow_midday, allow_powerhour, _fetch_last_with_fallback)
        ...
        try:
            for _r in (list(results_rev or []) + list(results_ride or []) + list(results_swing or []) + list(results_mss or [])):
                ...
                if sym and isinstance(last, (int, float)):
                    LAST_PRICE_CACHE[str(sym).upper().strip()] = float(last)
        except Exception:
            pass
